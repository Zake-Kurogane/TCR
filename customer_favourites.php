...
<head>
  ...
  <title>Favorites - TC Car Rental</title>
  ...
  <nav class="h-screen w-30 bg-white shadow-lg flex flex-col items-center pt-6 space-y-8 fixed">
        <div class="px-6">
          ...
          <nav class="flex flex-col space-y-8 text-gray-500 mt-14">
            ...
            <a href="customer_favourites.php" class="flex justify-center items-center space-x-2"><img src="img/heart (1).png" alt="Settings Icon" class="w-6 h-6"></a>
...
$sql = "
    SELECT 
        ...
        b.booking_status 
    FROM cars c 
    LEFT JOIN bookings b ON c.id = b.car_id AND b.user_id = ? 
    WHERE 1=1";
...
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
      // Check if the booking status is NULL or Approved/Pending/Declined
      ...
      if ($isBooked || is_null($bookingStatus)) {
          // Encode the car data for the data attribute
          $carData = json_encode([
              'id' => $row['id'],
              'status' => $carStatus,
          ...
          if ($isBooked) {
              // Check the specific booking status and set the class accordingly
              ...
              <div class="flex items-center justify-between mb-4">
                  ...
                  <div class="ml-auto favorite-container">
                      <button class="flex items-center favorite-button" onclick="toggleFavorite(' . $row['id'] . ', this); event.stopPropagation();">
                          <img src="img/heart (1).png" alt="favourite" class="w-6 h-6 heart-icon" data-favorited="false">
                      </button>
                  </div>
              </div>
